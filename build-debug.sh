...
"$GODOT" --headless --path "$PROJECT_DIR" --export-debug "Windows Desktop (Debug)" "$BUILD_DIR/NinjaCowboy.exe"

echo ""
echo "Debug build ready: build_debug/NinjaCowboy.exe"
echo "Run it from a terminal to see crash output:"
echo "  cd build_debug && ./NinjaCowboy.exe"
echo "  -- or for guaranteed console output: --"
echo "  cd build_debug && ./NinjaCowboy_console.exe"
